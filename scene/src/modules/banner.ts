import { Entity, GltfContainer, InputAction, Material, MeshCollider, MeshRenderer, PointerEventType, PointerEvents, Transform, engine, inputSystem } from "@dcl/sdk/ecs";
...
export class CustomPainting {
    ...
    constructor() {
        ...
        Transform.create(this.mainEntity,{position: Vector3.create(6,4,6), scale: Vector3.create(5,5,0.1)});
        ...
        const params = {pos: Vector3.create(0, 0, 0.7), rot: Quaternion.create(0.0, 0.0, 0.0), scale: Vector3.create(-1, 1, 1)};
        Transform.create(this.picture,{parent: this.mainEntity, position: params.pos, rotation: params.rot, scale: params.scale});
        this.invokePointer();
    }
    ...
    invokePointer(){
        PointerEvents.create(this.mainEntity,{pointerEvents: [
            {
