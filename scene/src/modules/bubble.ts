...
export class TextBubble {
    ...
    public invokeBubbleText(text: string, duration: number = 0) {
        ...
        let fontSize: number;
        if (textLength <= 100) {
            fontSize = 1; // Larger font for short text
        } else if (textLength <= 390) {
            fontSize = 0.5; // Medium font for medium length text
        }else if (textLength <= 550) {
            fontSize = 0.45; // Medium font for medium length text
        } else {
            fontSize = 0.4; // Smaller font for longer text
        }

        const mutableTextShape = TextShape.getMutable(this.frontTextEntity);
        mutableTextShape.text = text;
        mutableTextShape.fontSize = fontSize;
